...
const create = ({
    ...
    const updateConfig = async () => {
        ...
        fetch('https://trader.degiro.nl/login/secure/config', {
    ...
    const login = () => {
        ...
        if (oneTimePassword) {
            ...
            url += '/totp';
            console.log("añade one time password")
    ...
    const sendLoginRequest = (url, params) => {
        return fetch(url, {
            ...
            .then(res => {
                ...
                console.log("Login ok: " + session.id)
            })
            .then(updateConfig)
            .then(getClientInfo)
            .then(() => session);
